...
const openAIService = async (inputText) => {
  try {
    const response = await together.chat.completions.create({
      messages: [
        { role: 'system', content: 'Check input text for grammar and spelling errors from user input. Then write corrected sentence in the first line. Do not write any introductory or explanatory phrases.' },
...
const GrammarCheck = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const errorHighlights = highlightError({ input: inputText, output: result });
